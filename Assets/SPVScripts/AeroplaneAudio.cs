...
public class AeroplaneAudio : MonoBehaviour
    {
        ...
        private SPVPlaneController m_Plane;      // Reference to the aeroplane controller.
        private Rigidbody m_Rigidbody;
        private AudioMixerGroup amg;
        
        private void Awake()
        {
        AudioMixer am = Resources.Load<AudioMixer>("MainMixer");
        ...
        m_WindSoundSource.spatialBlend = 1;
        ...
        amg = am.FindMatchingGroups("PlaneSound")[0];
        m_EngineSoundSource.outputAudioMixerGroup = amg;
        amg = am.FindMatchingGroups("AudioFX")[0];
        m_WindSoundSource.outputAudioMixerGroup = amg;
        ...
        private void Update()
        {
        if (!m_Plane.SpeedChanging)
            return;
            // Find what proportion of the engine's power is being used.
            var enginePowerProportion = Mathf.InverseLerp(0, m_Plane.PlaneMaxSpeed, m_Plane.GetSpeed());
            ...
            m_EngineSoundSource.pitch += m_Plane.GetSpeed() *m_EngineFwdSpeedMultiplier;

            // Set the engine's volume to be proportional to the engine's current power.
            m_EngineSoundSource.volume = Mathf.InverseLerp(0, m_Plane.PlaneMaxSpeed*m_AdvancedSetttings.engineMasterVolume,
                                                         m_Plane.GetSpeed());
